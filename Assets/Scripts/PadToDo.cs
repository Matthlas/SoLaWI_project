/*
 Dringlichkeitsstufe 1 sehr hoch - 5 niedrig




make agents react to player (matze)                         5   done
Design city (marlena)                                       5   done
Make city accessible by player                              5   done
Debug UI dialogue (Eddie)                                   5   halfwegs done
Develop Questsystem (Eddie)                                 5   done  
make example Quest (Eddie)                                      done
quest anpassen                                              4   done
make beach (marlenski)
fix npc bug (matze)                                         4   done
Add more animation options: 
waving, walking, sitting (marlena)                          5   done
make UI match                                               3   done
Add weather                                                 1   nope
Add minimap (tut lowpoly) (Annika)                          2   done
Activate/deactivate Minimap (Annika)                        2
Spielstand speicher und fortsetzten (Annika)                5
startscreen(Annika)                                         5   done?
replace item images with 3D Models                          2   nope
gitarre in Hand nehmen                                      2   nope
pickup animation (matze)                                    3   done
urban gardening (matze)                                     2   done
Add NPCs city                                               3   done
Debug beds in NPC city (setInActive)                        5   done
add hand item to inventory (eddie)                          3   done
gießanimation                                               4   done
check submission conditions (marlena)                       
record video                                                5
build game                                                  5   done
upload game versions                                        5   done
Write GitHub Techincal Report                               5   done
Merge into main branch
Put Video link into github readme
Upload Github link to group folder
Write email with links

Feuerknistern                                               1   nope
animate water(marlena)                                      3   done
world building (matze)                                      3   done




Already done (and explanations):

game logic refinen                                          1       done
ernten debuggen  only if ready to harvest                   1       done
i für instructions                                          1       done
randbedingungen & z !< 0                                    1       done
animation player wieder hinzufügen (ohne jump)              1       done
pflanzen verschöenern                                       1       done
make plantgrow more beautiful?  the beet stays sooo small   1       done
In Inventory über Seedlistener Saattütenimages ändern       2       done
verschiedene pflanzen in logic                              2       done
        Use SeedListener to add vegetables, use Plant.Transform() to add growing states 
        (switch (vegetable)
            if size == 1 : change zylinder to vegetable)
                            +transform
            if size == 2: transform
            if size == 3: transform
remove vacant spaces in inventory                           2       done
CameraMovement Debuggen (auch weniger sensitiv?)            3       done
challenges einrichten                                       3       done
einzelnes Beet entfernen                                    3       done
erntezähler                                                 3       done
gitarrenmusik                                               4       done
wenn schaufel -> neues beet anlegen                         4
       Idee dazu: inaktive grüne Beetreihen die sich auch so heben, als könnte man mit denen
       interagieren. Wenn Dig() dann werden die aktiv. Wenn man überall Beete erstellen kann
       wird es so unübersichtlich...                                done
aufräumen (v.a. 3rd Party unused löschen)                   4       done
make day and night slowlier                                 5       done
waldimage an rändern einfügen?                              5       done


*/